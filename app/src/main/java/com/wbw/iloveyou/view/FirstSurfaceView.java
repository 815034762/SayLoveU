...
import com.wbw.iloveyou.util.Font16;
import com.wbw.iloveyou.util.Font24;
import com.wbw.iloveyou.util.Font32;
    ...
    class showWenZi extends Thread {
        ...
        public void run() {
            ...
            if (null != LoveApplication.info) {
                one = LoveApplication.info.getUsername();
                if (one.length() >= 2) {
                    two = one.substring(1, 2);
                } else {
                    two = one.substring(0, 1);
                }
            } else {
                one = spxml.getConfigSharedPreferences("first_name_1", spxml.getResourceString(R.string.first_et_1));
                two = spxml.getConfigSharedPreferences("first_name_2", spxml.getResourceString(R.string.first_et_2));
            }
            if (one.equals("")) {
                one = spxml.getResourceString(R.string.first_et_1);
            }
            ...
            } else if (one1.equals("郭")) {
                show_I(onestartx, onestarty, 20, 20, "array_guo.txt", bei, 2);
            } else if (one1.equals("莎")) {
                show_I(onestartx, onestarty, 20, 20, "array_sha.txt", bei, 2);
            } else {
            ...
            if (two.equals("")) {
                ...
                else if (two1.equals("琳"))
                    show_I(twostartx, twostarty, 20, 20, "array_lin.txt", bei, 2);
                else if (two1.equals("莎"))
                    show_I(twostartx, twostarty, 20, 20, "array_sha.txt", bei, 2);
    ...
    class showheart extends Thread {
            ...
            for (int i = 1; i < 51 && !isallstop; i++) {
                ...
                synchronized (holder) {
                    ...
                    try {
                        ...
                        //画中间的心
                        c.drawBitmap(m_m, start_x, start_y, p);
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
                        try {
        ...
        private void run_hua_heart() {
            ...
            for (int i = 0; i < maxh && !isallstop; i++) {
                ...
                //这里的13.5可以控制大小
                double a = 13.5 * (16 * Math.pow(Math.sin(b), 3));
                double d = -13.5
            ...
            for (int i = 1; i < 100 && !isallstop; i++) {
                ...
                synchronized (holder) {
                    ...
                    try {
                        c = holder.lockCanvas(rt);
                        //画中间的心
                        c.drawBitmap(bit, startx, starty, pa);
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
                        try {
                ...
                for (int i = 1; i < 21 && !isallstop; i++) {
                    ...
                    synchronized (holder) {
                        ...
                        try {
                            ...
                            //画中间的心
                            c.drawBitmap(love_middle_down, x, y, p);
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
                            // 结束锁定画图，并提交改变。
                            if(holder != null && null != holder.getSurface() &&  holder.getSurface().isValid()){
                                holder.unlockCanvasAndPost(c);
                            }
                        }
                        try {
                            ...
                            //画左边
                            c.drawBitmap(love_down_left, dl_x, dl_y, p);
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
                            try {
